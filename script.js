...
//MODAL WINDOW - adding event listeners to all the buttons from the node list to open the modal
btnsOpenModal.forEach(btn => btn.addEventListener('click', openModal));
//closing the modal if clicked on close button
btnCloseModal.addEventListener('click', closeModal);
//closing the modal if clicked outside the modal
overlay.addEventListener('click', closeModal);
//closing the modal if clicked on esc button
...
tabContainer.addEventListener('click', function (e) {
  //since it contains a span element as child
  const clickedTab = e.target.closest('.operations__tab');
  // console.log(clickedTab);
  //guard clause - in case tab container is clicked the function ends
  if (!clickedTab) return;

  //removing class on all
  tabs.forEach(t => t.classList.remove('operations__tab--active'));
  //adding on only 1
  clickedTab.classList.add('operations__tab--active');

  //removing classes
  const dataval = e.target.getAttribute('data-tab');
  tabsContent.forEach(c => c.classList.remove('operations__content--active'));
  //adding classes to current el
  document
    .querySelector(`.operations__content--${dataval}`)
    .classList.add('operations__content--active');
}); // or use clickedTab.dataset.tab
